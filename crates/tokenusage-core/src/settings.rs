...
impl AppSettings {
    pub fn apply_patch(&mut self, patch: SettingsPatch) {
        if let Some(value) = patch.language { self.language = value; }
        if let Some(value) = patch.theme { self.theme = value; }
        if let Some(value) = patch.visible_runtimes {
            self.visible_runtimes = value;
        }
        if self.visible_runtimes.is_empty() { self.visible_runtimes.push(RuntimeScope::Codex); }
        if let Some(value) = patch.selected_runtime { self.selected_runtime = value; }
        if !self.visible_runtimes.contains(&self.selected_runtime) { self.selected_runtime = self.visible_runtimes[0]; }
        if let Some(value) = patch.show_used_quota { self.show_used_quota = value; }
        ...
        if let Some(value) = patch.codex_data_directory { self.codex_data_directory = value; }
        if let Some(value) = patch.claude_data_directory { self.claude_data_directory = value; }
    }
}
...
impl SettingsStore {
    pub fn new(config_directory: PathBuf) -> Self { Self { path: config_directory.join("settings.json") } }
    pub fn load(&self) -> AppSettings {
        fs::read(&self.path).ok().and_then(|data| serde_json::from_slice(&data).ok()).unwrap_or_default()
    }
...
mod tests {
    ...
    fn settings_always_keep_one_visible_runtime() {
        ...
        settings.apply_patch(SettingsPatch { visible_runtimes: Some(vec![]), ..Default::default() });
    ...
    fn selected_runtime_falls_back_to_visible_runtime() {
        let mut settings = AppSettings::default();
        settings.apply_patch(SettingsPatch { visible_runtimes: Some(vec![RuntimeScope::ClaudeCode]), selected_runtime: Some(RuntimeScope::Codex), ..Default::default() });
