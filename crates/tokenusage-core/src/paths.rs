...
impl DataPaths {
    pub fn live(settings: &AppSettings) -> Self {
        let home_directory = env_path("USERPROFILE")
            .or_else(|| env_path("HOME"))
            .unwrap_or_else(|| PathBuf::from("."));
        let codex_directory = settings
            .codex_data_directory
            .as_ref()
            .map(PathBuf::from)
            .or_else(|| env_path("TOKENUSAGE_CODEX_DATA_DIR"))
            .unwrap_or_else(|| home_directory.join(".codex"));
        let claude_directory = settings
    ...
    pub fn codex_database_path(&self) -> Option<PathBuf> {
        [
            self.codex_directory.join("state_5.sqlite"),
            self.codex_directory.join("sqlite").join("state_5.sqlite"),
        ]
        .into_iter()
    ...
    pub fn codex_executable_candidates(&self, settings: &AppSettings) -> Vec<PathBuf> {
        ...
        if let Some(path) = settings.codex_executable_path.as_deref() {
            candidates.push(PathBuf::from(path));
        }
        if let Some(path) = env_path("TOKENUSAGE_CODEX_BIN") {
...
fn app_config_directory(home: &Path) -> PathBuf {
    if let Some(override_directory) = env_path("TOKENUSAGE_CONFIG_DIR") {
...
fn app_cache_directory(home: &Path) -> PathBuf {
    if let Some(override_directory) = env_path("TOKENUSAGE_CACHE_DIR") {
        return override_directory;
    }
    if cfg!(windows) {
