...
pub fn read(path: &Path) -> Result<(AppServerSnapshot, Vec<DiagnosticItem>)> {
    ...
    let mut child = command.spawn().context("unable to start codex app-server")?;
    ...
    while Instant::now() < deadline && complete.iter().any(|done| !done) {
        ...
        let Ok(line) = receiver.recv_timeout(timeout) else {
            break;
        };
        ...
        if value.get("error").is_some() {
            diagnostics.push(DiagnosticItem::warning(
                ...
                "Codex app-server did not return one account data source.",
            ));
            continue;
        }
    ...
    if !initialized {
        diagnostics.push(DiagnosticItem::warning(
            ...
            "Codex app-server did not complete initialization within 12 seconds.",
        ));
    } else if complete.iter().any(|done| !done) {
        diagnostics.push(DiagnosticItem::warning(
...
fn parse_window(value: Option<&Value>) -> Option<RateWindow> {
    ...
    Some(RateWindow::new(
        used_percent,
        value.get("windowDurationMins").and_then(number_i64),
        resets_at,
    ))
}
